...
void resourcesManager::draw(engine* Engine) {
	for (int i = 0; i < m_resources.size(); i++) {
		Engine->DrawString(m_beginX + m_stepsX/(m_resources.size() + 1) * i + m_padding, m_beginY + m_stepsY / 2 - engine::charHeight / 2, m_resourcesNames[i] + ": " + std::to_string(m_resources[i]));
	}
}
